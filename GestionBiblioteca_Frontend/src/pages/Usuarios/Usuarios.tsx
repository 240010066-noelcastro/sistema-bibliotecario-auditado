...
const [filterEstado, setFilterEstado] = useState('Todos');
  ...
  const saveAdminRecord = async () => {
    if (!adminFormData.NombreUsuario || !adminFormData.ApellidoPaterno || !adminFormData.CorreoElectronico) {
      return showToast("Nombre, Apellidos y Correo son obligatorios.", "danger");
    }
    setIsLoading(true);
    try {
      ...
      setShowAdminForm(false);
      setAdminFormData({ NombreUsuario: '', ApellidoPaterno: '', ApellidoMaterno: '', CorreoElectronico: '', Telefono: '' });
      await fetchPage(currentPage);
    } catch (error: any) {
      showToast(error.response?.data?.message || "Error al crear el administrador.", "danger");
    } finally {
      setIsLoading(false);
    }
  };
        ...
        <div className="usuarios-layout">
          ...
          {showAdminForm && (
            <div className="usuarios-form-card" style={{ borderLeft: '5px solid #582c83' }}>
              <h3 className="form-title" style={{ color: '#582c83' }}>Alta de Nuevo Administrador / Personal</h3>
              <div className="form-row margin-bottom-15">
